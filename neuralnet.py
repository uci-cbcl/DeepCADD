  ...
  def Train(self):
    ...
    while not stop:
      ...
      if self.EvalNow(step):
        ...
        valid_preds, valid_targets = self.Evaluate(validation=True, collect_predictions=collect_predictions)
        # Evaluate on test set.
        test_preds, test_targets = self.Evaluate(validation=False, collect_predictions=collect_predictions)
        if select_best:
          ...
          if valid_error < best_valid_error:
            ...
            best_valid_targets = valid_targets.copy()
            best_test_preds = test_preds.copy()
            best_test_targets = test_targets.copy()
        #for e in self.edge:
      ...
      if self.SaveNow(step):
        self.t_op.current_step = step
        self.CopyModelToCPU()
        util.WriteCheckpointFile(self.net, self.t_op,valid_preds=valid_preds,valid_targets=valid_targets,test_preds=test_preds,test_targets=test_targets)
        if dump_best:
          dump_best = False
          if select_model_using_error:
          ...
          util.WriteCheckpointFile(best_net, best_t_op, best=True,valid_preds=best_valid_preds,valid_targets=best_valid_targets,test_preds=best_test_preds,test_targets=best_test_targets)
