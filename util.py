...
def WriteCheckpointFile(net, t_op, best=False,valid_preds=None,valid_targets=None,test_preds=None,test_targets=None):
  ...
  if best:
    ...
    f.close()
    if(valid_preds!=None):
        valid_preds_file = checkpoint_file + '_valid_preds'
        np.save(valid_preds_file, valid_preds)
    if(valid_targets!=None):
        valid_targets_file = checkpoint_file + '_valid_targets'
        np.save(valid_targets_file, valid_targets)
    if(test_preds!=None):
        test_preds_file = checkpoint_file + '_test_preds'
        np.save(test_preds_file, test_preds)
    if(test_targets!=None):
        test_targets_file = checkpoint_file + '_test_targets'
        np.save(test_targets_file, test_targets)
  else:
    tag = 'LAST'
