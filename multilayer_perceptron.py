...
class BaseMultilayerPerceptron(six.with_metaclass(ABCMeta, BaseEstimator)):
    ...
    def _init_fit(self):
        ...
        if self.algorithm == "tanh":
            c = 1
        else:
            c = 4
        a = c*6/np.sqrt(self.n_features + self.n_hidden)
        self.coef_hidden_ = rng.uniform(
            -a, a, (self.n_features, self.n_hidden))
        self.coef_output_ = rng.uniform(-a, a, (self.n_hidden, self.n_outputs))
        self.intercept_hidden_ = rng.uniform(-a, a, self.n_hidden)
        self.intercept_output_ = rng.uniform(-a, a, self.n_outputs)
